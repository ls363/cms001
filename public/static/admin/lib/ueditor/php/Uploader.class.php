 ...
 * Date: 12-7-18
 * Time: 上午11: 32
 * Update: lichunguang 2022-09-25 19:40
 * UEditor编辑器通用上传类
 */
class Uploader
{
    ...
    private function checkMime(){
        $mimeExt = self::$extensionMimeList[$this->fileType] ?? '';
        if($this->mimeType == $mimeExt){
    ...
    private function getFilePath()
    {
        ...
        $rootPath = str_replace($_SERVER['PHP_SELF'], '', $_SERVER['SCRIPT_FILENAME']);
        if (substr($fullname, 0, 1) != '/') {
            $fullname = '/' . $fullname;
        }
        $url = $rootPath . $fullname;
        $url = str_replace('//', '/', $url);
        return $url;
    }
