...
class ContentLogic
{
    ...
    {
        return Content::query()->orderBy('sort', 'asc')->get();
    }
    ...
    public static function save($id, $data)
    {
        try {
            ...
            if ($id > 0) {
                unset($data['id']);
                Content::query()->where('id', $id)->update($data);
            } else {
                $seo_title = $data['seo_title'] ?? '';
                if(empty($seo_title)){
                    $data['seo_title'] = $data['title'];
                }
                $id = Content::insert($data);
            }
            return $id;
        } catch (\Exception $e) {
            return false;
    ...
    public static function setState(int $id, int $state){
        ...
        return Content::where('id', $id)->update(['state' => $state]);
    }
    ...
    public static function setField(int $id, string $field, int $state){
        ...
        return Content::where('id', $id)->update([$field => $state]);
    }
    ...
    {
        try {
            return Content::query()->whereIn('id', $ids)->delete();
        } catch (\Exception $e) {
            return false;
        }
    }
    ...
    {
        try {
            return Content::query()->whereIn('id', $ids)->update(['class_id' => $class_id]);
        } catch (\Exception $e) {
            return false;
        }
    }
